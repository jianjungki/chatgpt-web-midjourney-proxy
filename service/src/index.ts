...
router.post("/session", async (req, res) => {
	try {
		...
		if (!isHk)
			isHk =
				(process.env.LUMA_SERVER ?? "").toLocaleLowerCase().indexOf("-hk") > 0;
		if (!isHk)
			isHk =
				(process.env.VIGGLE_SERVER ?? "").toLocaleLowerCase().indexOf("-hk") >
				0;
...
app.use(
	...
	authV2,
	proxy(
		process.env.MJ_SERVER ? process.env.MJ_SERVER : "https://api.openai.com",
		{
			https: false,
			limit: "10mb",
	...
	if (process.env.FILE_SERVER) {
		...
		app.use("/openapi/v1/upload", authV2, upload.single("file"), (req, res) => {
			...
			if (req.file.filename)
				res.json({
					url: `/uploads/${formattedDate()}/${req.file.filename}`,
					created: Date.now(),
				});
...
app.use(
	"/sunoapi",
	authV2,
	proxy(process.env.SUNO_SERVER ?? API_BASE_URL, {
		https: false,
		limit: "10mb",
		proxyReqPathResolver(req) {
