...
const clearLimit = (req: Request, _res: Response) => {
  const ipAddress = getIp(req)
  bannedIPs[ipAddress] = 0
  ipErrorCount[ipAddress] = 0
}

const checkLimit = (req: Request, _res: Response) => {
  ...
  if (bannedIPs[ipAddress] && Date.now() < bannedIPs[ipAddress]) {
    ...
    logger.info('myIP', ipAddress, ipErrorCount[ipAddress])
  ...
  ipErrorCount[ipAddress] = ipErrorCount[ipAddress]
    ? ipErrorCount[ipAddress] + 1
    : 1
...
export const verify = async (req: Request, res: Response) => {
  try {
    ...
    const auth_secret_keys = process.env.AUTH_SECRET_KEY
...
export const auth = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (isNotEmptyString(AUTH_SECRET_KEY)) {
    try {
      ...
      if (
        !Authorization
          || !auth_secret_keys.includes(Authorization.replace('Bearer ', '').trim())
      )
...
export const regCookie = async (
  ...
  try {
    ...
    if (!outcome.success) {
      throw new Error(
        'Turnstile 错误,请刷新重试 | No access rights by Turnstile',
      )
    }
...
export const turnstileCheck = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
