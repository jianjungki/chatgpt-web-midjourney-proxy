		...
		<div class="p-4 space-y-4">
			<h2 class="text-xl font-bold">
				...
				<a
					...
					target="_blank"
				>
					({{ $t("mj.findVersion") }} {{ st.lastVersion }})</a
				>
				<a
					v-else-if="st.lastVersion"
					class="text-gray-500"
					href="https://github.com/Dooy/chatgpt-web-midjourney-proxy"
					target="_blank"
				>
					({{ $t("mj.yesLastVersion") }})</a
				>
			</h2>
			<div class="p-2 space-y-2 rounded-md bg-neutral-100 dark:bg-neutral-700">
				<p v-text="$t('mj.infoStar')"></p>
			</div>
			<p>{{ $t("setting.api") }}：{{ config?.apiModel ?? "-" }}</p>
			<div
v-if="isChatGPTAPI"
class="flex items-center justify-between">
				<div>{{ $t("setting.monthlyUsage") }}：{{ config?.usage ?? "-" }}</div>
				<div>
					{{ $t("mj.totalUsage") }}：{{
						config?.hard_limit_usd ? (+config?.hard_limit_usd).toFixed(2) : "-"
					}}
				</div>
				<div>{{ $t("setting.balance") }}：{{ config?.remaining ?? "-" }}</div>
			</div>
			<p v-if="!isChatGPTAPI">
