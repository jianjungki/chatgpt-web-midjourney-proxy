...
async function selectFileVideo(input: any) {
	...
	let d: any = await viggleFetch(
		getMyProUrl("/asset/video/" + f.value.imageID),
		formData,
		{ upFile: true },
	)
	mlog("d Video", d)
...
async function selectFile(input: any) {
	try {
		...
		let d: any = await viggleFetch(getMyProUrl("/asset/image"), formData, {
			upFile: true,
		})
		mlog("d Image", d)
		if (d.data) {
...
watch(
	() => isHK.value,
	(n) => saveMyDate(n && st.value.version == "pro"),
)
watch(
	() => st.value.version,
	() => saveMyDate(isHK.value && st.value.version == "pro"),
)
...
size="small" />
		...
		<div class="pt-2 flex justify-center items-center w-full relative">
			...
			<div
v-if="st.imgSrc"
class="absolute right-1 top-3 z-40">
				<NButton
					strong
					secondary
					round
					...
					type="success"
					@click="clear(2)"
					>{{ $t("common.clear") }}</NButton
				>
			</div>
			<div
				class="h-[180px] w-full overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer"
				@click="fsRef.click()"
			>
				<NImage
v-if="st.imgSrc"
...
class="pt-2">
		...
		<div
v-else
class="pt-2 flex justify-center items-center w-full">
			<input
				ref="vsRef"
				type="file"
				style="display: none"
...
v-text="t('dance.info')"></div>
	</div>

	<n-drawer
		v-model:show="st.showImg"
