...
import type { ViggleTask } from '@/api/viggleStore'
import { viggleStore } from '@/api/viggleStore'
...
<template>
      ...
      <div v-for="(item, index) in list" :key="index" class="relative" @mousemove="st.pIndex = index" @mouseout="st.pIndex = -1">
        <div class="relative flex items-center justify-center bg-white bg-opacity-10 rounded-[16px] overflow-hidden aspect-[16/8.85] ">
          ...
          <div v-else class=" text-center">
            <NButton v-if="!item.last_feed || ((new Date().getTime()) - item.last_feed) > 20 * 1000" size="small" type="primary" @click="FeedViggleTask(item.taskID)">
              {{ $t('video.repeat') }}
            </NButton>
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex justify-end items-center pt-1">
            ...
            <NButtonGroup size="tiny">
              <NButton size="tiny" round ghost @click="TaskDown(item)">
  ...
  <div v-else class="w-full h-full flex justify-center items-center">
    <NEmpty description="请先创作才有跳舞视频列表" />
  </div>
</template>
