...
import type { ViggleTemplate, tagInfo } from '@/api/viggle'
import { viggleFetch } from '@/api/viggle'
...
const loadTag = async () => {
  const d = await viggleFetch('/template2/tag')
  mlog('tags', d)
  if (d.data) {
...
const searchQ = async (q: string, tagID: string) => {
  const url = `/template2?page=1&pageSize=48&searchKeyword=${encodeURIComponent(q)}&tagID=${tagID}&type=0`
  const d = await viggleFetch(url)
  mlog('searchQ', d)
  myList.value = []
  ...
  <div class="w-full h-full p-4">
    <div class="flex items-center justify-start line-clamp-1 pb-4">
      <div v-for="v in tagOption" class="m-1 cursor-pointer" @click="goSearch(v)">
        <NButton v-if="v.name == pp.q" strong round size="small" type="success">
          {{ v.name }}
        </NButton>
      ...
      <div v-for="(v, k) in myList" class="  mb-2 ">
        <div class="relative h-[180px]" @mousemove="st.qindex = k">
          <div v-if="st.qindex == k" class="absolute right-2 top-2 z-40">
            <NButton rounded size="small" type="success" @click="useVideo(v)">
              {{ $t('dance.use') }}
            </NButton>
          </div>
          <div class="   absolute w-full h-full top-0 left-0 z-1">
            <video
              ...
              class="  rounded-lg bg-[#242424] object-contain w-full h-full transition-all" :src="v.processedURL"
            />
          </div>
        </div>
