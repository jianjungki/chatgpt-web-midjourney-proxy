...
async function selectFile(input: any) {
	try {
		...
		const d: any = await sunoFetch("/uploads/audio", {
		...
		mlog("uploaded ");
		const f: any = await sunoFetch(
			"/uploads/audio/" + d.id + "/upload-finish",
			{
				upload_type: "file_upload",
				upload_filename: input.target.files[0].name,
			},
		);
		mlog("finish ", f);
		...
		mlog("uploadFetch ", fd);
		const ft: any = await sunoFetch(
			"/uploads/audio/" + d.id + "/initialize-clip",
			{},
		);
		mlog("clip ", ft);
...
<template>
	<NTag type="success" size="small" round>
		...
		<span class="cursor-pointer" @click="fsRef.click()" v-else>{{
			$t("suno.upMps")
		}}</span>
	</NTag>
