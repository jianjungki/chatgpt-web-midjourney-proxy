		...
		>
			...
			<div class="flex-1 pl-2">
				...
				>
					...
					<div class="opacity-80 line-clamp-1 max-w-[320px]" v-if="item.tags">
						{{ item.tags.join(", ") }}
					</div>
				</div>
				<div
				...
				>
					<div
						...
						v-if="item.audio_conditioning_type"
					>
						{{
							item.audio_conditioning_type == "precede"
								? t("mj.ud_precede")
								: t("mj.ud_continuation")
						}}
					</div>
					<div
					...
					<n-popconfirm
						@positive-click="() => deleteGo(item.id)"
						placement="bottom"
					>
						<template #trigger
					...
					<SvgIcon
						...
						@click="goPlay(item)"
					/>
					<a :href="item.song_path" download target="_blank"
