...
const canPost = computed(() => {
	...
	if (st.value.type == "description") {
		mlog(
			"des: ",
			des.value.gpt_description_prompt,
			des.value.make_instrumental,
		)
		return (
			cs.value.title &&
			(des.value.gpt_description_prompt || des.value.make_instrumental)
		)
	}
	return true
})
...
const generateLyrics = () => {
	...
	sunoFetch("/generate/lyrics/", { prompt })
		.then(async (r: any) => {
			mlog("lyrics", r)
...
animated>
			...
			<n-tab-pane
name="description"
:tab="$t('suno.description')">
				<div class="pt-1">
					<n-input
v-model:value="cs.title"
:placeholder="$t('suno.titlepls')">
						<template #prefix>
				...
				<div class="pt-4 flex justify-between">
					...
					<div>
						<n-switch
v-model:value="des.make_instrumental"
size="small">
							<template #checked>
								{{ $t("suno.noneedly") }}
							</template>
...
:placeholder="$t('suno.titlepls')">
				...
				<div class="pt-4">
					<n-input
v-model:value="cs.tags"
:placeholder="$t('suno.stylepls')">
						<template #prefix>
...
size="small">
				...
				<div class="pt-1">
					<n-input
						v-model:value="cs.prompt"
						:disabled="des.make_instrumental"
						:placeholder="$t('suno.lypls')"
						type="textarea"
						size="small"
						:autosize="{ minRows: 3, maxRows: 12 }"
					/>
...
round
								><span
class="cursor-pointer"
@click="cs.continue_clip_id = ''"
									>清除</span
								></NTag
							>
						</div>
						<n-slider
							v-model:value="cs.continue_at"
							:step="1"
							:max="Math.ceil(exSuno.metadata.duration)"
						>
							<template #thumb>
								<div
									class="bg-[--n-fill-color] text-[9px] border-[0px] px-1 list-none rounded-md"
								>
									{{ cs.continue_at }}s
								</div>
							</template>
					...
					<div class="pt-1">
						...
						>
									...
									<template #placeholder>
										<div class="w-full h-full justify-center items-center flex">
											<SvgIcon
												...
												class="text-[40px] text-green-300"
											></SvgIcon>
										</div>
									</template>
								</n-image>
							...
							<div class="flex-1 pl-2">
								<div
									class="flex justify-between line-clamp-1 w-full cursor-pointer"
								>
									<h3>{{ exSuno.title }}</h3>
								...
								>
									...
									<div
										v-if="exSuno.major_model_version"
										class="text-[8px] flex items-center border-[1px] border-gray-500/30 px-1 list-none rounded-md"
									>
...
:disabled="!canPost"
@click="generate()"
					><SvgIcon icon="ri:music-fill" /> {{ $t("suno.generate") }}</NButton
				>
			</div>
		</div>
		<div
			v-if="st.type == 'custom'"
			class="pt-4 text-[12px]"
			v-text="t('suno.info')"
		></div>
	</div>
</template>
