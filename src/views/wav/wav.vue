...
onMounted(() => {
	st.value.apikey = localStorage.getItem("_t_apikey") || "";
	st.value.baseUrl =
		localStorage.getItem("_t_baseurl") || "wss://api.openai.com/v1/realtime";
});
</script>
...
<template>
	...
	<div class="p-4">
		<div class="pb-2">
			<NInput v-model:value="st.baseUrl" placeholder="base url" />
		</div>
		<div class="pb-2">
			<NInput v-model:value="st.apikey" placeholder="api key" />
		</div>
		<div class="space-x-2">
			<NButton type="primary" @click="go" v-if="!st.isConnect">连接</NButton>
			<NButton type="primary" @click="disconnectConversation" v-else
				>断开</NButton
			>
			<NButton
				type="primary"
		...
		<div v-else class="flex justify-between items-baseline">
			<section class="w-full">
				<div
					...
					:class="conversationItem.role === 'assistant' ? ['text-right'] : []"
				>
					...
					<div
						class="flex"
						:class="
							conversationItem.role === 'assistant' ? ['justify-end'] : []
						"
					>
						<audio
