...
:key="index">
				<section
					...
					:class="{ active: luma.aspect_ratio == item.label }"
					@click="luma.aspect_ratio = item.label"
				>
					<div
		...
		<div class="pt-1">
			<div class="flex justify-between items-center">
				<div>
					<n-switch
v-model:value="luma.expand_prompt"
size="small">
						<template #checked> Enhance prompt </template>
						<template #unchecked> Enhance prompt </template>
					</n-switch>
				</div>
				<div>
					<n-switch
v-model:value="luma.loop"
size="small">
						<template #checked> Loop </template>
						<template #unchecked> Loop </template>
					</n-switch>
				</div>
			</div>
		</div>
...
class="pt-1">
			...
			>
				<video
					v-if="exLuma.video?.url || exLuma.video?.download_url"
					:src="
						exLuma.video?.download_url
							? exLuma.video?.download_url
							: exLuma.video?.url
					"
					loop
					...
					@error="$event.target.src = exLuma.video?.url"
				></video>
			</div>
		</div>
...
class="text-center">{{ $t("video.selectimg") }}</div>
				...
				<div class="pl-2">
					...
					>
						<img
v-if="luma.image_end_url"
:src="luma.image_end_url" />
						<div
v-else
class="text-center">{{ $t("video.endImg") }}</div>
					</div>
				</div>
			</div>
		</div>
