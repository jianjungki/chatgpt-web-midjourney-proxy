...
const runway = ref<{
	...
	seed: number;
	text_prompt: string;
}>({ image_prompt: "", seed: 1675247627, text_prompt: "" })
const st = ref({
	isDo: false,
	uploading: false,
	version: "gen2",
...
const canPost = computed(() => {
	return (
		(runway.value.image_prompt != "" || runway.value.text_prompt != "") &&
		!st.value.isDo
	)
})
		...
		let gen3 = {
			...
			options: {
				...
				extended_from_task_id:
					exRunway.value && exRunway.value.id ? exRunway.value.id : undefined,
				init_video:
					exRunway.value &&
					exRunway.value.artifacts &&
					exRunway.value.artifacts[0].url
						? exRunway.value.artifacts[0].url
						: undefined,
			},
			//    "asTeamId": 17511575
...
onMounted(() => {
	...
	st.value.version = gptServerStore.myData.RRUNWAY_VERSION
		? gptServerStore.myData.RRUNWAY_VERSION
		: "gen2"
})
					...
					<n-popover trigger="hover">
						<template #trigger>
							<div class="line-clamp-1">
								{{ $t("video.extend") }}:
								<template v-if="exRunway.options.text_prompt">{{
									exRunway.options.text_prompt
								}}</template>
								<template v-else>{{
									exRunway.options.gen2Options?.text_prompt
										? exRunway.options.gen2Options.text_prompt
										: exRunway.name
								}}</template>
							</div>
						...
						<div v-if="exRunway.taskType == 'gen3a_turbo'">
							Version: Gen-3-turbo
						</div>
						<div v-if="exRunway.taskType == 'gen2'">Version: Gen-2</div>
						<div v-if="exRunway.createdAt">
							createdAt: {{ new Date(exRunway.createdAt).toLocaleString() }}
						</div>
						<div
v-if="exRunway.options.text_prompt"
class="max-w-[300px]">
							{{ exRunway.options.text_prompt }}
						</div>
		...
		<div
v-if="st.version != 'gen2'"
class="pt-1">
			<n-select
v-model:value="st.time"
:options="timeOption"
size="small" />
		</div>
...
class="text-center">{{ $t("video.selectimg") }}</div>
					...
					<div
						v-if="runway.image_prompt && st.version != 'gen2'"
						class="absolute bottom-[-5px] right-[-15px]"
					>
						<a
						...
						<NSwitch
v-else
v-model:value="st.image_as_end_frame"
size="small">
...
@click="clearInput()">{{
		...
		<div
class="pt-2 text-[12px]"
v-text="$t('video.runwayinfo')"></div>
	</div>
</template>
