...
import { NButton, NInput, NPopover, NSelect, NSwitch, NTag, useMessage } from 'naive-ui'
...
const generate = async () => {
  ...
  const seed = getRandomInt(1375247627, 3975247627)
  try {
    const obj = {
      taskType: 'gen2',
      internal: false,
...
<template>
  <div>
          ...
          <NPopover trigger="hover">
            <template #trigger>
              <div class="line-clamp-1">
                ...
                <template v-else>
                  {{ exRunway.options.gen2Options?.text_prompt ? exRunway.options.gen2Options.text_prompt : exRunway.name }}
                </template>
              </div>
            </template>
            ...
            <div v-if="exRunway.taskType == 'gen3a'">
              Version: Gen-3
            </div>
            <div v-if="exRunway.taskType == 'gen3a_turbo'">
              Version: Gen-3-turbo
            </div>
            <div v-if="exRunway.taskType == 'gen2'">
              Version: Gen-2
            </div>
            <div v-if="exRunway.createdAt">
              createdAt: {{ new Date(exRunway.createdAt).toLocaleString() }}
            </div>
            <div v-if="exRunway.options.text_prompt" class=" max-w-[300px]">
              {{ exRunway.options.text_prompt }}
            </div>
            <div class=" max-w-[300px]">
              {{ exRunway.options.gen2Options?.text_prompt ? exRunway.options.gen2Options.text_prompt : exRunway.name }}
    ...
    <div v-if="st.version != 'gen2'" class="pt-1">
      <NSelect v-model:value="st.time" :options="timeOption" size="small" />
    </div>
      ...
      <div class="flex justify-between  items-end">
        <div class=" relative">
          ...
          <div class="h-[80px] w-[80px] overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer" @click=" fsRef.click()">
            <SvgIcon v-if="st.uploading" icon="line-md:uploading-loop" class="text-[60px] text-green-300" />
        ...
        <div class="text-right ">
          <div class="pb-1 flex justify-between items-center">
            <NTag v-if="runway.text_prompt != '' || runway.image_prompt != '' || exRunway" type="success" size="small" round>
