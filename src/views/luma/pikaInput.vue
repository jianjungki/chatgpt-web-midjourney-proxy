...
const createVideo = async () => {
  const sb = pika.value.image ? { ...img2v } : { ...txt2v }
...
<template>
  <div class="p-2">
    ...
    <div class="pt-1">
      <NSelect v-model:value="pika.mv" :options="mvOption" size="small" />
    </div>
    ...
    <section class="pt-2 flex justify-end items-end">
      <div v-if="pika.image || pika.prompt" class=" cursor-pointer pr-2" @click="clearInput">
      ...
      <div class="text-right">
        <NButton :loading="st.isLoading" type="primary" :disabled="!pika.prompt" @click="createVideo()">
          {{ $t('video.generate') }}
        </NButton>
      </div>
    </section>
