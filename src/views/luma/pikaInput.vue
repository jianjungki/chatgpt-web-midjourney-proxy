...
:key="item.value">
				<section
					...
					@click="pika.aspectRatio = item.value"
				>
					<div
...
size="small" />
		...
		<section class="pt-1 flex justify-between items-center">
			<div>{{ $t("mj.nohead") }}</div>
			<NInput
				v-model:value="pika.negativePrompt"
				size="small"
				class="!w-[70%]"
				clearable
...
class="text-center">{{ $t("video.selectimg") }}</div>
				...
				<div>
					<NPopover trigger="hover">
						<template #trigger>
							<div
								class="h-[80px] w-[150px] relative overflow-hidden rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer"
							>
								<template v-if="pika.pe_index > -1">
									<img :src="ezOption[pika.pe_index].poster" />
									<div class="absolute top-1 right-1 text-white/75 text-[14px]">
...
:key="index">
									...
									>
										<video
											class="h-[72px] w-full rounded-md object-cover"
											:src="item.video"
											:poster="item.poster"
											autoplay
											loop
										...
										></video>
										<div
											class="absolute top-1 right-1 text-white/75 text-[14px]"
										>
											{{ item.title }}
										</div>
									</div>
