...
import { NButton, NButtonGroup, NEmpty, NPopconfirm, useMessage } from 'naive-ui'
import { pikaFeed } from '@/api/pika'
import type { PikaTask } from '@/api/pikaStore'
          ...
          <template v-if="item.videos.length > 0">
            ...
            <template v-else-if="(!item.last_feed || ((new Date().getTime()) - item.last_feed) > 20 * 1000) ">
              <div class="w-full h-[200px] justify-center items-center flex">
                <NButton size="small" type="primary" @click="pikaFeed(item.id)">
                  {{ $t('video.repeat') }}
                </NButton>
              </div>
            </template>
        ...
        <div class="flex justify-between items-center">
          ...
          <section class="flex justify-end items-center pt-1">
            <NButtonGroup size="tiny">
              <NButton v-if="item.videos[0].resultUrl" size="tiny" round ghost>
