...
const create = async () => {
	...
	if (luma.value.image_end_url)
		f.value.promptImage.push({
			uri: luma.value.image_end_url,
			position: "last",
		})
...
:key="index">
					<section
						...
						:class="{ active: item.value == f.ratio }"
						@click="f.ratio = item.value"
					>
						<div
...
class="text-center">{{ $t("video.endImg") }}</div>
		...
		<section class="mb-4 flex justify-between items-end">
			<div class="relative">
				<div
					v-if="luma.image_end_url || luma.image_url || f.promptText"
