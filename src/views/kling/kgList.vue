...
const toList2 = () => {
	list2.value = list.value.map((v, k) => {
		let url =
			v.data.task_result?.images?.[0]?.url ||
			v.data.task_result?.videos?.[0]?.url ||
			""
...
class="p-4">
		...
		>
			<template #item="{ item, index }">
				<div
					class="bg-white dark:bg-[#24272e] rounded-md overflow-hidden cursor-pointer group/item relative"
				>
					<div v-if="'failed' == item.task.data.task_status">
						<div
							class="w-full h-[200px] justify-center items-center flex text-center"
						>
					...
					<template
						v-else-if="
							(!item.task.last_feed ||
								new Date().getTime() - item.task.last_feed > 20 * 1000) &&
							!item.src
						"
					>
...
@click="getFeed(item)">{{
					...
					<template v-else>
						<div
							v-if="item.task.cat != 'image' && item.src"
						...
						<div
v-else
class="w-[200px] h-[200px]"></div>

						<section
							v-if="
								!item.src ||
								(item.isLoad == 0 && st.isStart && item.task.cat == 'image')
							"
					...
					>
						<div class="p-3 flex justify-between items-baseline">
							<div class="line-clamp-2 text-[13px]">
								<template v-if="item.task.prompt">{{
									item.task.prompt
								}}</template>
							</div>
							<div>
