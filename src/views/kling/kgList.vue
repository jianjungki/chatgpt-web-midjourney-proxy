...
import { NButton, NEmpty, NImage, NPopconfirm, NSpin, useMessage } from 'naive-ui'
import type { ViewCard } from 'vue-waterfall-plugin-next/dist/types/types/waterfall'
import KgImage from './kgImage.vue'
import type { KlingTask } from '@/api/klingStore'
        ...
        <div class="bg-white dark:bg-[#24272e] rounded-md   overflow-hidden cursor-pointer group/item relative">
          ...
          <template v-else>
            <div v-if="item.task.cat != 'image' && item.src" style="padding-bottom: 0%;">
              <video v-if="item.src" :src="item.src" loop playsinline disableremoteplayback disablepictureinpicture :controls="st.pIndex == index" class="w-full h-full object-cover" />
            ...
            <div v-else class="w-[200px] h-[200px]" />
