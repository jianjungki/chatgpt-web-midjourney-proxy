...
<template>
  <div class="overflow-y-auto  h-full ">
    ...
    <section class="mb-4 flex justify-between items-center">
      <div>{{ $t('mj.mode') }}</div>
      <NSelect v-model:value="f.mode" size="small" :options="modeOptions" class="!w-[70%]" />
    </section>
    <section class="mb-4 flex justify-between items-center">
      <div>{{ $t('mj.duration') }}</div>
      <NSelect v-model:value="f.duration" size="small" :options="durationOptions" class="!w-[70%]" />
    </section>
    <section class="mb-4 flex justify-between items-center">
      <div>{{ $t('mj.camera_type') }}</div>
      <NSelect v-model:value="st.camera_type" size="small" :options="cameraOption" class="!w-[70%]" />
    </section>
    ...
    <section class="mb-4 flex justify-between items-end">
      <div class="relative">
        <div v-if="f.image || f.prompt || f.image_tail" class=" cursor-pointer pb-2" @click="clearInput">
      ...
      <div class="text-right">
        <NButton :loading="st.isLoading" type="primary" :disabled="!f.prompt" @click="createImg()">
          {{ $t('video.generate') }}
        </NButton>
      </div>
    </section>
