		...
		try {
			...
			if (!base64) {
				const ubase64 = await url2base64(
					`https://wsrv.nl/?url=${encodeURIComponent(pp.item.src)}`,
					key,
				)
...
<template>
	<LazyImg
		v-if="st.isLoad"
		:url="st.uri_base64 ? st.uri_base64 : pp.item.src"
		@success="emit('kgSuccess')"
		@click="emit('kgClick', { base64: st.uri_base64, src: pp.item.src })"
	/>
</template>
