...
<template>
  <div class="overflow-y-auto bg-[#fafbfc]   dark:bg-[#18181c] h-full ">
    ...
    <section class="mb-4 flex justify-between items-center">
      ...
      <NSelect v-model:value="f.model" :options="'V_2,V_2_TURBO,V_1,V_1_TURBO'.split(',').map(v => ({ label: v, value: v }))" size="small" class="!w-[70%]" :clearable="true" />
    </section>
    <section class="mb-4 flex justify-between items-center">
      <div>Seed</div>
    ...
    <section class="mb-4 flex justify-between items-end">
      ...
      <div class="text-right">
        <div v-if="st.image_url || f.prompt " class=" cursor-pointer pb-2" @click="clearInput">
          <NTag type="success" size="small" :bordered="false" round>
            <span class="cursor-pointer">{{ $t('video.clear') }}</span>
          </NTag>
        </div>

        <NButton type="primary" :disabled="!f.prompt" @click="createImg()">
          {{ $t('mjchat.imgcreate') }}
        </NButton>
      </div>
    </section>
