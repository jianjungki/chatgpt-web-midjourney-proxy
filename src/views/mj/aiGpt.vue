		...
		if (n == "gpt.submit" || n == "gpt.whisper") {
			...
			if (n == "gpt.whisper") {
				...
				try {
					...
					updateChatSome(+uuid2, dataSources.value.length - 1, {
						text: whisper.text,
						opt,
					});
			...
			let message = [
				{ role: "system", content: getSystemMessage(+st.value.uuid) },
				...historyMesg,
			];
			if (isNewModel(model)) {
				message = [...historyMesg];
			...
			subTTS({
				...
				.then((d) => {
					...
					setTimeout(() => {
						homeStore.setMyData({
							act: "playtts",
							actData: { saveID: d.saveID },
						});
					}, 100);
				})
				.catch((e) => {
					let emsg = JSON.stringify(
						e.reason ? JSON.parse(e.reason) : e,
						null,
						2,
					);
