...
import {
  ...
  , getInitChat, getSystemMessage, isCanBase64Model, isNewModel, isTTS, localSaveAny, mlog,
  subModel,
  subTTS,
  whisperUpload,
} from '@/api'
...
watch(() => homeStore.myData.act, async (n) => {
  if (n == 'gpt.submit' || n == 'gpt.whisper') {
    ...
    if (dd.fileBase64 && dd.fileBase64.length > 0) {
      ...
      try {
        const images = await localSaveAny(JSON.stringify({ fileName: dd.fileName, fileBase64: dd.fileBase64 }))
        mlog('key', images)
