...
const doAdd = async () => {
  ...
  if (d.error != 0)
    ...
    // data.gpts
  const msg = `${t('mjchat.success3')} ${d.data?.gpts?.name}`
  ms.success(msg)
  mlog('rz=', d)
}
...
<template>
  <NInput v-model:value="st.q" round :placeholder="$t('mjchat.addPlaceholder')" clearable @keydown.enter="doAdd()">
    <template #prefix>
  ...
  <div v-if="st.gid" class="px-4 text-white/25 text-[12px]">
    gid:{{ st.gid }}
  </div>
</template>
