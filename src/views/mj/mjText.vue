		...
		if (chat.value.opt?.imageUrl) {
			...
			if (!base64 || isFlash) {
				const ubase64 = await url2base64(
					mjImgUrl(chat.value.opt?.imageUrl),
					key,
				);
...
<template>
	<div v-if="st.isLoadImg">
		...
		<template v-else-if="chat.opt?.progress">
			...
			>
				{{ chat.opt?.promptEn }} (<span v-html="chat.opt?.action"></span>)
			</div>
			<NImage
	...
	>
		...
		<NButton type="primary" v-if="chat.opt?.imageUrl"
			><a :href="mjImgUrl(chat.opt?.imageUrl)" target="_blank">{{
				$t("mjchat.openurl")
			}}</a></NButton
		>
	</div>
