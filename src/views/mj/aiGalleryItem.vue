	...
	for (let i in rz) {
		...
		try {
			if (v.image_url) {
				...
				if (!base64) {
					await url2base64(v.image_url, key)
					mlog("图片已保存>>", key)
				} else {
...
const loadImagFormLocal = async () => {
	...
	let rz = d
		.filter((v: any) => v.opt && v.opt.imageUrl)
		.map((v: any) => {
				...
				>
					<div class="p-3">
						<div class="line-clamp-2 text-[13px]">
							...
							<NTag
								v-else-if="item.action == 'SWAP_FACE'"
								type="success"
								size="small"
								round
								:label="$t('mjchat.face')"
							></NTag>
							<NTag
								v-else-if="item.action == 'BLEND'"
								type="success"
								size="small"
								round
								:label="$t('mjchat.blend')"
							></NTag>
							<NTag
v-else
type="success"
