...
<template>
	...
	>
		<div class="flex justify-between items-center w-full">
			<div
class="text-blue-500 cursor-pointer mr-8"
@click="download">
				<SvgIcon icon="ri:download-2-fill"></SvgIcon>
			</div>
...
@click="go">
				<span
					...
					v-text="chat.opt?.duration.toFixed(2) + `s`"
				></span>
				<span
v-else
v-text="st.fileName"></span>
				<div class="rotate-90 cursor-pointer">
					<SvgIcon
v-if="st.isLoad == 1"
icon="svg-spinners:wifi"></SvgIcon>
					<SvgIcon
v-else
icon="mdi:wifi"></SvgIcon>
				</div>
			</div>
		</div>
	</div>
