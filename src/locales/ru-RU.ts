export default {
	...
	mj: {
		...
		temperatureInfo:
			"При увеличении значения (temperature) ответы становятся более случайными",
		top_p: "Верхняя вероятность выборки",
		top_pInfo:
			"(top_p) аналогично случайности, но не следует изменять вместе с температурой",
		presence_penalty: "Свежесть темы",
		presence_penaltyInfo:
			"При увеличении значения (presence_penalty) увеличивается вероятность расширения на новые темы",
		...
		micRec:
			"Начать запись, пожалуйста, говорите! Запись автоматически остановится, если 2 секунды не будет звука.",
		...
		needImg:
			"Пожалуйста, загрузите эталонное изображение, чтобы это заработало!",
		seed: "Число семени 1~2147483647",
		klingInfo:
			"Описание: <li>1. Высокое качество стоит в 3.5 раза дороже</li> <li>2. 10 секунд стоит в 2 раза дороже</li> <li>3. Последний кадр должен иметь эталонное изображение для действия</li>",
		...
		rtsetting:
			"Пожалуйста, настройте сервер. В настоящее время Realtime поддерживает только удаленные службы; для локальных услуг свяжитесь с автором.",
		...
		rtservererror2:
			"Запись не поддерживается, возможно, это связано с устройством!",
		...
		ud_info:
			"Примечание: <ul><li>1. Udio-32 имеет короткую продолжительность</li><li>2. Udio-130 стоит вдвое дороже Udio-32</li><li>3. В подсказке можно указать стиль, описание и т. д.</li></ul>",
		...
		upImg2:
			'<span><b>Загрузить изображение</b><br/>Эта модель поддерживает распознавание изображений<br>Примечание: будут дополнительные расходы на изображения<br/>Форматы: jpeg jpg png gif</span><p>Поддерживает перетаскивание</p> <p class="pt-2"><b>Загрузить MP3 MP4</b> <br>Автоматически вызовет модель whisper-1<br>Форматы: mp3 mp4 mpeg mpga m4a wav webm</p>',
		rml_info:
			"Примечание:<ul><li>1. Должно быть изображение</li><li>2. Модель имеет только одну gen3a_turbo</li><li>3. Цена за 10 секунд вдвое выше, чем за 5 секунд</li></ul>",
		rml_heng: "Пейзаж",
		rml_shu: "Портрет",
	},
	mjset: {
	...
	video: {
		...
		runwayinfo:
			"Объяснение: <ul><li>1. У изображений и видеороликов Runway есть срок действия.</li><li>2. Пожалуйста, сохраните файл MP4 локально в течение 30 минут после создания видео.</li></ul>",
