...
export const isFileMp3 = (filename: string) => {
	let arr = ".mp3, .mp4, .mpeg, .mpga, .m4a, .wav, .webm".split(/[, ]+/gi)
	mlog("fileIsMp3", arr)
...
export const canVisionModel = (model: string) => {
	...
	if (
		["gpt-4-all", "gpt-4-v", "gpt-4v", "gpt-3.5-net", "gpt-4o-all"].indexOf(
			model,
		) > -1
	)
		return true
...
export const isCanBase64Model = (model: string) => {
	...
	if (
		...
		(model.indexOf("gemini") > -1 && model.indexOf("1.5") > -1)
	) {
		return true
	}
	...
	if (homeStore.myData.session.customVisionModel) {
		homeStore.myData.session.customVisionModel
			.split(/[ ,]+/gi)
			.map((v: string) => {
